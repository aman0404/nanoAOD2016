...
const std::unordered_map<std::string, CUTS> Analyzer::cut_num = {
  ...
  //Aman
  {"NMuon1Jet1Combinations", CUTS::eMuon1Jet1},         {"NMuon1Jet2Combinations", CUTS::eMuon1Jet2},
  {"NMuon2Jet1Combinations", CUTS::eMuon2Jet1},         {"NMuon2Jet2Combinations", CUTS::eMuon2Jet2},
...
//  std::cout<< "$$$$$$$$$$$$ AMAN CHANGE EVENTS $$$$$$$$$$$$"<<std::endl;   
//std::cout <<"----------------------------------------------"<<std::endl;
...
//Aman
  ...
  initializeElectronFakeFactor(infiles);
...
void Analyzer::create_fillInfo() {
  ...
  fillInfo["FillElectron1Jet2"] =   new FillVals(CUTS::eElec1Jet2, FILLER::Dilepjet, _Electron, _Jet);
...
//Aman
  fillInfo["FillMuon1Jet1"] =   new FillVals(CUTS::eMuon1Jet1, FILLER::Dilepjet, _Muon, _Jet);
  fillInfo["FillMuon1Jet2"] =   new FillVals(CUTS::eMuon1Jet2, FILLER::Dilepjet, _Muon, _Jet);
  fillInfo["FillMuon2Jet1"] =   new FillVals(CUTS::eMuon2Jet1, FILLER::Dilepjet, _Muon, _Jet);
  fillInfo["FillMuon2Jet2"] =   new FillVals(CUTS::eMuon2Jet2, FILLER::Dilepjet, _Muon, _Jet);
...
//Aman
  getGoodLeptonJetCombos(*_Muon, *_Jet, CUTS::eRMuon1, CUTS::eRJet1, CUTS::eMuon1Jet1, distats["Muon1Jet1"],syst);
  getGoodLeptonJetCombos(*_Muon, *_Jet, CUTS::eRMuon1, CUTS::eRJet2, CUTS::eMuon1Jet2, distats["Muon1Jet2"],syst);
  getGoodLeptonJetCombos(*_Muon, *_Jet, CUTS::eRMuon2, CUTS::eRJet1, CUTS::eMuon2Jet1, distats["Muon2Jet1"],syst);
  getGoodLeptonJetCombos(*_Muon, *_Jet, CUTS::eRMuon2, CUTS::eRJet2, CUTS::eMuon2Jet2, distats["Muon2Jet2"],syst);
...
//Aman
  read_info(filespace + "MuonJet_info.in");
...
void Analyzer::getGoodGen(const PartStats& stats) {
  ...
  for(size_t j = 0; j < _Gen->size(); j++) {
   ...
   //   if((id == 11 || id == 13) && abs(_Gen->pdg_id[_Gen->genPartIdxMother[j]]) != 24) continue; //added for Z' signal considerations 11.28.18, 01.16.19
     //if((id == 11 || id == 13) && abs(_Gen->pdg_id[_Gen->genPartIdxMother[j]]) == 23) continue;
...
void Analyzer::getGoodRecoLeptons(const Lepton& lep, const CUTS ePos, const CUTS eGenPos, const PartStats& stats, const int syst) {
  ...
  for(auto lvec: lep) {
	...
	//if (lep.type == PType::Tau){std::cout << "pass cuts: " << passCuts << " first "<<firstIso<< "second "<<secondIso<< " getIso "<< lep.get_Iso(i, firstIso, secondIso)<<std::endl;}
...
//Aman
////      std::cout<<"END OF A EVENT"<<std::endl;
         if(cut == "DoDiscrByTransRegion") passCuts = passCuts && isNotTransRegion(lvec.Eta());
...
//////Aman
////Transition region in ECAL
//
bool Analyzer::isNotTransRegion(float etaValue){

return (fabs(etaValue < 1.4442) || fabs(etaValue > 1.566));
...
//Aman
  void Analyzer::initializeElectronFakeFactor(std::vector<std::string> infiles){

	TFile hist_fake("Pileup/electron_fakerate.root");
	Electronfake_h =dynamic_cast<TH2D*>(hist_fake.FindObjectAny("fake_electron_h"));
	 hist_fake.Close();
...
void Analyzer::initializeMuonFakeFactor(std::vector<std::string> infiles){
 ...
 Muonfake_h =dynamic_cast<TH2D*>(hist_fakeMuon.FindObjectAny("fake_muon_h"));
 hist_fakeMuon.Close();
}
...
double Analyzer::MuonFakeRate(){
	...
	if(active_part->at(CUTS::eRMuon1)->size()&&active_part->at(CUTS::eRMuon2)->size()){			
...
//Aman
 double Analyzer::ElectronFakeRate(){

	 double Electronfakerate = 1.;
                double temp1 = 1.;

	if(active_part->at(CUTS::eRMuon1)->size()&&active_part->at(CUTS::eRElec1)->size()){

	double elePt = _Electron->p4(active_part->at(CUTS::eRElec1)->at(0)).Pt();
                                double eleEta = _Electron->p4(active_part->at(CUTS::eRElec1)->at(0)).Eta();
                                int binx = Electronfake_h->GetXaxis()->FindBin(elePt);
                                int biny = Electronfake_h->GetYaxis()->FindBin(eleEta);
                                temp1 = Electronfake_h->GetBinContent(binx,biny);
                                Electronfakerate = temp1;

				}

		return Electronfakerate;
				}
...
void Analyzer::fill_histogram() {
  ...
  if(!isData){
	...
	if(isData || !isData){
	wgt = 1.;
	if(distats["Run"].bfind("ApplyElectronFakeRate")) wgt *= ElectronFakeRate();
