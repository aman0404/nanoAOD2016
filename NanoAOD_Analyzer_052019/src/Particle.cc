...
//  cbIDele2=tmp;
  ...
  std::bitset<8> tmp=elec1.dmap.at("DiscrByHLTID");
  cbHLTIDele1=tmp;
...
bool Taus::get_Iso(int index, double onetwo, double flipisolation) const {
 ...
 double value = MVAoldDM[index];
	//std::cout<<value<<" check "<<std::endl; 
  if(!flipisolation){
	
    //std::cout<<tau_isomin_mask<<"  "<<tau_iso<<"   "<<(tau_isomax_mask& tau_iso)<<"   "<<  (tau_isomax_mask& tau_iso).count()<<std::endl; 
